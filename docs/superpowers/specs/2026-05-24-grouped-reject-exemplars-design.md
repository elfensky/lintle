...
The get-or-create form is deliberate: `setdefault(category, [])` would
evaluate `[]` on every call (CPython evaluates positional arguments before
checking key membership), allocating an empty list per reject that is
...
get-or-create form allocates only on the first occurrence of a category.
...
- `test_exemplars_bucketed_per_category_with_complete_broken_catalog` —
  ...
  N >> `_PER_CATEGORY_EXEMPLAR_BOUND` rejects of one category through
  `process_file` in `"clean"` mode; assert (a) the per-category bucket holds
  exactly `_PER_CATEGORY_EXEMPLAR_BOUND` entries, (b) `stats.quarantined_count == N`
  and `stats.reject_categories[cat] == N`, (c) the sidecar header reads
...
- `test_validate_mode_bucket_caps_per_category` — the evolved form of
  ...
  N > `_PER_CATEGORY_EXEMPLAR_BOUND` rejects of one category; assert the
  bucket caps at `_PER_CATEGORY_EXEMPLAR_BOUND` and `stats.quarantined_count == N`.
...
- `test_internal_error_category_bucketed_like_data_defects` —
  ...
  `RejectCategory.INTERNAL_ERROR` bucket capped at N like every data defect.
...
- `test_main_validate_renders_grouped_exemplars` — drive a file with at least
  two distinct defect categories through `cli.main(["validate", ...])`; assert
  the captured stdout contains (a) the category heading literal
  `"  checksum-mismatch ("` (two-space indent + category + space + open paren),
