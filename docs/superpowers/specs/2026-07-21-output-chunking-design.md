...
| `dedup` | `dedup/import.NNNNN.txt`, `dedup/notes.NNNNN.jsonl` (single writer each) | reads `cleaned/` as chunk sets |
...
stems from `{stem}.NNNNN.cleaned.txt` names (glob `*.cleaned.txt`, strip the
`.NNNNN.cleaned.txt` tail, dedupe); `iter_file()` streams a stem's ordered chunk set.
`dedup` and `verify` both reuse it; `diff` gets the same treatment for `report.jsonl`.
The per-worker findings **shards** in `.shards/<stem>.findings.jsonl` stay whole
intermediate files — only the final concatenated `report.jsonl` is chunked, at concat
time in the single main process (so no cross-worker coordination).
